...
export const ChosenCar =() =>{
    ...
    return (
        ...
        <Html position={[-0.6, -1.5, 0]}>
          ...
          {rotationPaused ? (
            <button onClick={()=>handleStartRotation()} className="rounded-full px-4 py-2">
              Showroom Mode
            </button>
          ) : (
            <button onClick={()=>handlePauseRotation()} className="rounded-full px-4 py-2">
              Pause Rotation
            </button>
          )}
            </div>
