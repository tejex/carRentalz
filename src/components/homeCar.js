...
export const Test =() =>{
    ...
    return (
        ...
        <Html position={[-0.6, -0.1, 0]}>
          ...
          {rotationPaused ? (
            <button onClick={()=>handleStartRotation()} className="bg-green-500 border px-4 py-2 text-white rounded-3xl">
              Showroom Mode
            </button>
          ) : (
            <button onClick={()=>handlePauseRotation()} className="bg-red-500 border px-4 py-2 text-white rounded-3xl">
              Pause Rotation
            </button>
          )}
            </div>
